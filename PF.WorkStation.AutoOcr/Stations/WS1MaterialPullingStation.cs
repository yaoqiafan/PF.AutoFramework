...
namespace PF.WorkStation.AutoOcr.Stations
{
    ...
    /// <summary>工位1拉料工站记忆参数，持久化夹爪状态与作业进度。</summary>
    public class WS1MaterialPullingMemoryParam : StationMemoryBaseParam
    {
    ...
    ///
    /// <para>跨工站协同：</para>
    /// 通过 <see cref="IStationSyncService"/> 与 <see cref="WS1FeedingStation"/> (上下料 Z 轴)
    ...
    public class WS1MaterialPullingStation : StationBase<WS1MaterialPullingMemoryParam, Station1PullingStep>
    {
        ...
        public override async Task ExecuteInitializeAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    _logger.Warn($"[{StationName}] 工位1已屏蔽，跳过拉料模组初始化。");
                ...
                if (IsGripperHoldingMaterial(persistedStep))
                {
                    ...
                    if (!res.HasValue)
                    {
                        ...
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.InitCheckFailed, "初始化失败，判断夹爪的物料状态异常");
                        return;
                    }
                    if (res.Value)
                    {
                        _logger.Error($"[{StationName}] 初始化失败，记忆中夹爪的状态为有料，实际检测为无料，检查轨道物料状态");
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.InitCheckFailed, "初始化失败，记忆中夹爪状态为有料、实际检测为无料，请检查轨道物料状态");
                        return;
                    }
                    ...
                    if (!result.IsSuccess)
                    {
                        ...
                        TriggerAlarm(
                            string.IsNullOrEmpty(result.ErrorCode)
                                ? AlarmCodesExtensions.WS1Pulling.GripperCloseFailed
                                : result.ErrorCode,
                            "初始化失败，关闭夹爪失败");
                        return;
                    }
                    if (!await _pullingModule.CheckWafeSizeControl(_cachedRecipe.WafeSize, token))
                    {
                        ...
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.InitCheckFailed, "初始化失败，当前轨道尺寸与物料尺寸不匹配");
                        return;
                    }
                    ...
                    if (!await _pullingModule.WaitHomeDoneAsync(_pullingModule.YAxis, token: token))
                    {
                        ...
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.InitPullingFailed, "初始化失败，Y轴回零异常");
                        return;
                    }
                    ...
                    if (!await _pullingModule.MoveDetection(_cachedRecipe.WafeSize, token))
                    {
                        ...
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.MoveInitialFailed, "初始化失败，Y轴移动到待机位异常");
                        return;
                    }
                }
                // 退料阶段（传感器不可靠）：仅确保夹爪闭合
                ...
                else
                {
                    ...
                    if (!result.IsSuccess)
                    {
                        ...
                        TriggerAlarm(
                            string.IsNullOrEmpty(result.ErrorCode)
                                ? AlarmCodesExtensions.WS1Pulling.GripperOpenFailed
                                : result.ErrorCode,
                            "初始化失败，打开夹爪失败");
                        return;
                    }
                    ...
                    {
                        _logger.Error($"[{StationName}] 初始化失败，Y轴移动到待机位异常。");
                        TriggerAlarm(AlarmCodesExtensions.WS1Pulling.MoveInitialNoScanFailed, "初始化失败，Y轴移动到待机位异常");
                        return;
                    }
                }
            ...
            {
                // 初始化被取消（超时/外部中止）：非故障，不弹具体报警，交由上层处理
                Fire(MachineTrigger.Error);
                throw;
            }
            catch (Exception ex)
            {
                // 兜底异常也必须带错误码触发报警，否则会被当作 CascadeAlarm 滤掉而不弹窗
                _logger.Error($"[{StationName}] 初始化异常：{ex.Message}");
                TriggerAlarm(AlarmCodesExtensions.WS1Pulling.InitCheckFailed, $"初始化异常：{ex.Message}");
                throw;
            }
            ...
            else
            {
                ...
                // 异步步序值不合法时回退到起点
                if (!Enum.IsDefined((Station1PullingStep)persistedStep) || persistedStep >= 100000)
                {
                    restoreStep = Station1PullingStep.等待允许取料;
                    ...
                    switch (_currentStep)
                    {
                        ...
                        case Station1PullingStep.移动到取料位:
                            ...
                            if (moveResult.IsSuccess)
                            {
                                ...
                                _currentStep = Station1PullingStep.检查料盒当前层是否有物料;
                            }
                            else
                            {
                        ...
                        case Station1PullingStep.检查料盒当前层是否有物料:
                            ...
                            {
                                RouteToError(Station1PullingStep.检查料盒当前层是否有物料异常, Station1PullingStep.检查夹爪无料异常步序1, checkgipperresult.ErrorCode);
                            }
                            break;
                        ...
                        case Station1PullingStep.检测叠料:
                            ...
                            if (await _pullingModule.CheckStackedPieces(token))
                            {
                                ...
                                MemoryParam.PersistedStep = (int)Station1PullingStep.检测叠料;
                                FlushMemory();
                        ...
                        case Station1PullingStep.移动到检测位:
                            ...
                            if (detectMoveResult.IsSuccess)
                            {
                                ...
                                _currentStep = Station1PullingStep.检测物料正反;
                            }
                            else
                            {
                            ...
                            break;
                        case Station1PullingStep.检测物料正反:
                            ...
                            {
                                RouteToError(Station1PullingStep.物料正反检测报警, Station1PullingStep.等待允许送料, dirResult.ErrorCode);
                            }
                            break;
                        ...
                        case Station1PullingStep.扫码识别:
                            ...
                            var checkResult = await WSDataModule.CheckCodeAsync(E_WorkSpace.工位1, scanResult.Data, token);
                        ...
                        case Station1PullingStep.判断带片:
                            ...
                            if (await _pullingModule.CheckGipperInsidePro(token))
                            {
                                ...
                                MemoryParam.PersistedStep = (int)Station1PullingStep.判断带片;
                                FlushMemory();
                        ...
                        case Station1PullingStep.物料正反检测报警:
                        case Station1PullingStep.检查料盒当前层是否有物料异常:
