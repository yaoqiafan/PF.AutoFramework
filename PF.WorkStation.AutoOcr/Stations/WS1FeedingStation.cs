    ...
    public class WS1FeedingStation : StationBase<WS1FeedingMemoryParam, Station1FeedingStep>
    {
        ...
        private async Task<bool> HandleResumeInProgressAsync(CancellationToken token)
        {
            ...
            if (!filterResult.IsSuccess)
            {
                ...
                TriggerAlarm(AlarmCodesExtensions.WS1Feeding.AlgorithmException, filterResult.ErrorMessage);
                return false;
            }
            ...
            {
                // 物料应在轨道上，但扫描显示不在
                Station1FeedingStep.阻塞等待物料回退完成 => (!isMaterialOnTrack && filterResult.Data.Count == MemoryParam.TotalLayerCount) ||
            ...
            // 根据上次中断的步骤，决定从哪里继续
            if ((savedStep == Station1FeedingStep.阻塞等待物料回退完成 && isMaterialOnTrack) || (savedStep == Station1FeedingStep.等待物料拉出完成 && isMaterialOnTrack))
            {
            ...
            else if (savedStep == Station1FeedingStep.等待物料拉出完成 && !isMaterialOnTrack)
            {
                _currentStep = Station1FeedingStep.判断物料可拉出条件;
                _resumeStep = Station1FeedingStep.判断物料可拉出条件;
            }

            else if (savedStep == Station1FeedingStep.阻塞等待物料回退完成 && !isMaterialOnTrack)
            {
                _currentStep = Station1FeedingStep.计算下一层位置;
                _resumeStep = Station1FeedingStep.计算下一层位置;
            }
            else if (savedStep == Station1FeedingStep.计算下一层位置)
            {
                    ...
                    switch (_currentStep)
                    {
                        ...
                        case Station1FeedingStep.验证尺寸与配方是否匹配:
                            ...
                            if (_detectedWaferSize == _cachedRecipe.WafeSize)
                            {
                                ...
                                _currentStep = Station1FeedingStep.切换阵列配方尺寸;
                            }
                            else
                            {
                        ...
                        case Station1FeedingStep.切换阵列配方尺寸:
                            ...
                            else
                            {
                                ...
                                RouteToError(Station1FeedingStep.切换阵列配方尺寸失败, Station1FeedingStep.等待按下工位1启动按钮, switchResult.ErrorCode);
                            }
                            break;
                        ...
                        case Station1FeedingStep.算法过滤层数:
                            ...
                            if (filterResult.IsSuccess)
                            {
                                ...
                                else
                                {
                                    ...
                                    FlushMemory();
                                    _dataModule?.InitializeSlotStates(E_WorkSpace.工位1, _layersToProcess);
                        ...
                        case Station1FeedingStep.切换到指定层:
                            ...
                            {
                                RouteToError(Station1FeedingStep.Z轴运动超时, Station1FeedingStep.切换到指定层, switchToLayerres.ErrorCode);
                            }
                            break;
                        ...
                        case Station1FeedingStep.判断物料可拉出条件:
                            ...
                            else
                            {
                                ...
                                RouteToError(Station1FeedingStep.拉料互锁失败_挡杆未打开, Station1FeedingStep.等待按下工位1启动按钮);
                            }
                            break;

                        case Station1FeedingStep.等待物料拉出完成:
                            CurrentStepDescription = "等待物料拉出完成...";
                            _dataModule?.SetInspectingSlot(E_WorkSpace.工位1, _layersToProcess[_currentLayerIndex]);
                            _dataModule?.UpdateSlotStatus(E_WorkSpace.工位1, _layersToProcess[_currentLayerIndex], WaferSlotStatus.Inspecting);
                        ...
                        case Station1FeedingStep.通知操作员下料:
                            ...
                            // 发布事件通知 UI 弹出确认弹窗
                            _eventAggregator.GetEvent<OperatorUnloadRequestedEvent>().Publish("工位1");

                            try
                            {
                                // 等待人工确认完成下料
                                await _sync.WaitAsync(nameof(WorkstationSignals.工位1人工下料完成), token, scope: E_WorkStation.工位1上下料工站.ToString()).ConfigureAwait(false);
                            }
                            finally
                            {
                        ...
                        case Station1FeedingStep.寻层算法过滤异常:
                            TriggerAlarm(AlarmCodesExtensions.WS1Feeding.AlgorithmException, "寻层算法过滤异常");
                            _currentStep = _resumeStep;
                            break;

                        case Station1FeedingStep.初始化上料状态失败:
                            var initErrCode = _cachedErrorCode ?? AlarmCodesExtensions.WS1Feeding.InitFeedingStateFailed;
                            TriggerAlarm(initErrCode, "上料状态初始化失败（Z/X轴运动到待机位失败）");
                            _cachedErrorCode = null;
                            _currentStep = _resumeStep;
                            break;

                        case Station1FeedingStep.切换阵列配方尺寸失败:
                            var switchSizeErrCode = _cachedErrorCode ?? AlarmCodesExtensions.WS1Feeding.SwitchArrayRecipeSizeFailed;
