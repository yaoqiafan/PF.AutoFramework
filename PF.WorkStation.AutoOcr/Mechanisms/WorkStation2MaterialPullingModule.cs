    ...
    public class WorkStation2MaterialPullingModule : BaseMechanism
    {
        ...
        public async Task<bool> ChangeWafeSizeControl(E_WafeSize wafesize, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("工位2轨道有晶圆，请先清除轨道物料再执行尺寸切换。");
                }

                if (wafesize == E_WafeSize._8寸)
                {
                    // 先断开对立侧气路，再给目标侧供气，防止电磁阀或气缸串气形成憋气死锁
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸伸出, false)) throw new Exception($"输入 {E_OutPutName.晶圆轨道右调宽气缸伸出} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸收回, true)) throw new Exception($"输入 {E_OutPutName.晶圆轨道右调宽气缸收回} 操作失败");

                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸伸出, false)) throw new Exception($"输入 {E_OutPutName.夹爪右X轴气缸伸出} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸缩回, true)) throw new Exception($"输入 {E_OutPutName.夹爪右X轴气缸缩回} 操作失败");
                ...
                {
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸收回, false)) throw new Exception($"输入 {E_OutPutName.晶圆轨道右调宽气缸收回} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸伸出, true)) throw new Exception($"输入 {E_OutPutName.晶圆轨道右调宽气缸伸出} 操作失败");

                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸缩回, false)) throw new Exception($"输入 {E_OutPutName.夹爪右X轴气缸缩回} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸伸出, true)) throw new Exception($"输入 {E_OutPutName.夹爪右X轴气缸伸出} 操作失败");
        ...
        public async Task<bool> OpenWafeGipper(CancellationToken token = default)
        {
            ...
            {
                // 先断开闭合气路，再打开张开气路
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右闭合, false)) throw new Exception($"操作输出信号 {E_OutPutName.夹爪气缸右闭合} 失败");
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右张开, true)) throw new Exception($"操作输出信号 {E_OutPutName.夹爪气缸右张开} 失败");
            ...
            {
                throw new Exception($"等待输入信号 {E_InPutName.晶圆夹爪右气缸张开} 超时，请检查气路。");
            }
            catch (Exception ex)
            {
                _logger.Warn(ex.Message);
        ...
        public async Task<bool> CloseWafeGipper(CancellationToken token = default)
        {
            ...
            try
            {
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右张开, false)) throw new Exception($"操作输出信号 {E_OutPutName.夹爪气缸右张开} 失败");
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右闭合, true)) throw new Exception($"操作输出信号 {E_OutPutName.夹爪气缸右闭合} 失败");
                ...
                if (!res1.HasValue) throw new Exception($"获取输入信号 {E_InPutName.晶圆夹爪右铁环有无检测} 失败");

                // 注意逻辑：此传感器如果为 true，可能代表"无料"或"透光"，需根据实际接线逻辑确认
                if (res1.Value)
                {
                    throw new Exception($"{E_InPutName.晶圆夹爪右铁环有无检测} 传感器感应异常，未检测到铁环物料。");
                }
                return true;
            }
            catch (OperationCanceledException) when (timeoutcts.IsCancellationRequested)
            {
                throw new Exception($"等待输入信号：{E_InPutName.晶圆夹爪右气缸闭合} 超时。");
            }
            catch (Exception ex)
            {
                _logger.Warn(ex.Message);
        ...
        public async Task<bool> MoveInitial(CancellationToken token = default)
        {
            try
            {
                ...
                if (await MoveMultiAxesToPointsAsync(new[] { (_yAxis, nameof(YAxisPoint.待机位置)) }, await ParamService.GetParamAsync<int>(E_Params.AxisMoveTimeout.ToString()), token: token))
                {
                    ...
                    if (res1.Value)
                    {
                        throw new Exception($"{E_InPutName.晶圆夹爪右铁环有无检测} 状态异常：机构已复位但检测到残留物料，请人工确认是否带料。");
                    }
                    return true;
                }
                throw new Exception($"[{MechanismName}] 移动到待机位失败");
        ...
        public async Task<bool> PutOverMove(CancellationToken token = default)
        {
            try
            {
                ...
                if (await MoveMultiAxesToPointsAsync(new[] { (_yAxis, nameof(YAxisPoint.取出安全位置)) }, await ParamService.GetParamAsync<int>(E_Params.AxisMoveTimeout.ToString()), token: token))
                {
                    ...
                    if (!res1.Value)
                    {
                        throw new Exception($"{E_InPutName.晶圆夹爪右铁环有无检测} 状态异常：卸料后依然检测到物料粘连。");
                    }
                    return true;
                }
