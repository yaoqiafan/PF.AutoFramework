...
{
    /// <summary>
    /// 【工位2】推拉晶圆模组 (Wafer Pulling Mechanism - Station 2)
    ///
    /// <para>物理架构：</para>
    /// 包含一个控制水平推拉的 Y 轴、一套多尺寸自适应的气动夹爪与变轨气缸，
    /// 以及光源控制器与工业扫码枪。对应右侧物理硬件。
    ///
    /// <para>软件职责：</para>
    ...
    public class WS2MaterialPullingModule : BaseMechanism
    {
        #region Enums (轴点枚举定义)
        ...
        /// 初始化工位2推拉晶圆模组
        /// </summary>
        public WS2MaterialPullingModule(
            IHardwareManagerService hardwareManagerService,
            IParamService paramService,
            IContainerProvider provider,
            ILogService logger)
            : base(E_Mechanisms.工位2推拉晶圆模组.ToString(), hardwareManagerService, paramService, logger)
        {
            Provider = provider;
        }
        ...
        protected override async Task<bool> InternalInitializeAsync(CancellationToken token)
        {
            ...
            // ① 延迟解析硬件实例
            _yAxis = HardwareManagerService?.GetDevice(E_AxisName.工位2拉料Y轴.ToString()) as IAxis;
            if (_yAxis == null)
            {
                _logger.Error($"[{MechanismName}] 未找到Y轴 '{E_AxisName.工位2拉料Y轴}'，请确认硬件配置。");
                return false;
            }
            ...
            _codeScan = HardwareManagerService?.GetDevice(E_ScanCode.工位2扫码枪.ToString()) as IBarcodeScan;
            if (_codeScan == null)
            {
                _logger.Error($"[{MechanismName}] 未找到扫码枪 '{E_ScanCode.工位2扫码枪}'，请确认硬件配置。");
                return false;
            }
        ...
        public async Task<MechResult> InitializeFullingAsync(CancellationToken token = default)
        {
            ...
            else
            {
                ...
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.InitPullingFailed, "初始化拉料流程失败，Y轴运动到待机位失败");
            }
        }
        ...
        public async Task<MechResult<bool>> CheckTrackIsMaterial(CancellationToken token = default)
        {
            ...
            _logger.Info($"[{MechanismName}] 开始检查右侧轨道是否残留晶圆物料...");

            // 读取IO信号 (替换为右侧检测)
            bool? res1 = _io.ReadInput((int)E_InPutName.晶圆轨道右晶圆在位检测1);
            bool? res2 = _io.ReadInput((int)E_InPutName.晶圆轨道右晶圆在位检测2);
            ...
            if (res1.HasValue && res2.HasValue)
            {
                ...
                _logger.Info($"[{MechanismName}] 右侧轨道物料检查完成，当前状态：{(hasMaterial ? "有物料残留" : "无物料")}。");
            ...
            {
                // 找出具体是哪个传感器读取失败，方便排查
                string failedSensor = !res1.HasValue ? nameof(E_InPutName.晶圆轨道右晶圆在位检测1) : nameof(E_InPutName.晶圆轨道右晶圆在位检测2);

                _logger.Error($"[{MechanismName}] 右侧轨道物料检查失败，未能成功读取 {failedSensor} 信号。");

                // 返回硬件报警码
                return MechResult<bool>.Fail(AlarmCodes.Hardware.IoGetError, $"读取右侧轨道物料检测信号({failedSensor})失败");
            }
        }
        ...
        public async Task<MechResult<bool>> CheckWafeSizeControl(E_WafeSize wafesize, CancellationToken token = default)
        {
            token.ThrowIfCancellationRequested(); // 【新增】入口取消检查
            CheckReady();
            _logger.Info($"[{MechanismName}] 开始检查右侧轨道与夹爪气缸是否处于 [{wafesize}] 的正确状态...");
            ...
            {
                trackSensor = E_InPutName.晶圆轨道右调宽气缸缩回;
                gripperSensor = E_InPutName.晶圆夹爪右8寸气缸缩回;
            }
            else
            {
                trackSensor = E_InPutName.晶圆轨道右调宽气缸打开;
                gripperSensor = E_InPutName.晶圆夹爪右12寸气缸打开;
            }
            ...
            if (trackRes.HasValue && gripperRes.HasValue)
            {
                ...
                _logger.Info($"[{MechanismName}] 右侧气缸 [{wafesize}] 状态检查完成，当前状态：{(isCorrectState ? "到位" : "未到位/异常")}。");
            ...
            else
            {
                ...
                _logger.Error($"[{MechanismName}] 右侧气缸状态检查失败，未能成功读取 {failedSensor} 信号。");
                return MechResult<bool>.Fail(AlarmCodes.Hardware.IoGetError, $"读取右侧气缸检测信号({failedSensor})失败");
            }
        }
        ...
        public async Task<MechResult> ChangeWafeSizeControl(E_WafeSize wafesize, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeTrackHasMaterial, "工位2轨道有晶圆，请先清除轨道物料再执行尺寸切换");
                }

                if (wafesize == E_WafeSize._8寸)
                {
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸伸出, false)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.晶圆轨道右调宽气缸伸出} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸收回, true)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.晶圆轨道右调宽气缸收回} 操作失败");

                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸伸出, false)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.夹爪右X轴气缸伸出} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸缩回, true)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.夹爪右X轴气缸缩回} 操作失败");

                    while (true)
                    {
                        linktoken.ThrowIfCancellationRequested(); // 【新增】循环内取消嗅探
                        await Task.Delay(1, linktoken);
                        bool? res1 = _io.ReadInput((int)E_InPutName.晶圆轨道右调宽气缸缩回);
                        bool? res2 = _io.ReadInput((int)E_InPutName.晶圆夹爪右8寸气缸缩回);
                ...
                {
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸收回, false)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.晶圆轨道右调宽气缸收回} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.晶圆轨道右调宽气缸伸出, true)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.晶圆轨道右调宽气缸伸出} 操作失败");

                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸缩回, false)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.夹爪右X轴气缸缩回} 操作失败");
                    if (!_io.WriteOutput((int)E_OutPutName.夹爪右X轴气缸伸出, true)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"{E_OutPutName.夹爪右X轴气缸伸出} 操作失败");

                    while (true)
                    {
                        linktoken.ThrowIfCancellationRequested(); // 【新增】循环内取消嗅探
                        await Task.Delay(1, linktoken);
                        bool? res1 = _io.ReadInput((int)E_InPutName.晶圆轨道右调宽气缸打开);
                        bool? res2 = _io.ReadInput((int)E_InPutName.晶圆夹爪右12寸气缸打开);
                        if (res1 == true && res2 == true) break;
                    }
                    return MechResult.Success();
                }
                ...
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, $"晶圆尺寸输入错误: {wafesize}");
                }
            }
            catch (OperationCanceledException)
            {
                if (timeoutcts.IsCancellationRequested) // 【精简点】合并异常路由
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderTimeout, "切换晶圆尺寸操作气缸超时，请检查气压或传感器状态");
                }
                _logger.Warn($"[{MechanismName}] 切换晶圆尺寸操作被外部取消");
                throw;
            }
            catch (Exception ex)
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.ChangeSizeCylinderFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> OpenWafeGipper(CancellationToken token = default)
        {
            ...
            try
            {
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右闭合, false)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperOpenCylinderFailed, $"操作输出信号 {E_OutPutName.夹爪气缸右闭合} 失败");
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右张开, true)) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperOpenCylinderFailed, $"操作输出信号 {E_OutPutName.夹爪气缸右张开} 失败");

                while (true)
                {
                    linktoken.ThrowIfCancellationRequested(); // 【新增】循环内取消嗅探
                    await Task.Delay(1, linktoken);
                    bool? res1 = _io.ReadInput((int)E_InPutName.晶圆夹爪右气缸张开);
                    if (res1 == true) break;
                }
                return MechResult.Success();
            }
            catch (OperationCanceledException)
            {
                if (timeoutcts.IsCancellationRequested) // 【精简点】合并异常路由
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperOpenTimeout, $"等待输入信号 {E_InPutName.晶圆夹爪右气缸张开} 超时，请检查气路");
                }
                _logger.Warn($"[{MechanismName}] 夹爪张开操作被外部取消");
                throw;
            }
            catch (Exception ex)
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperOpenCylinderFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> CloseWafeGipper(CancellationToken token = default)
        {
            ...
            try
            {
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右张开, false))
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperCloseCylinderFailed, "夹爪气缸右张开IO操作失败");
                if (!_io.WriteOutput((int)E_OutPutName.夹爪气缸右闭合, true))
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperCloseCylinderFailed, "夹爪气缸右闭合IO操作失败");

                while (true)
                {
                    linktoken.ThrowIfCancellationRequested(); // 【新增】循环内取消嗅探
                    await Task.Delay(1, linktoken);
                    bool? res = _io.ReadInput((int)E_InPutName.晶圆夹爪右气缸闭合);
                    if (res == true) break;
                }
                ...
                if (res1.Value)
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperCloseNoRing, "夹爪闭合后未检测到铁环物料（空夹）");
                }
                return MechResult.Success();
            }
            catch (OperationCanceledException)
            {
                if (timeoutcts.IsCancellationRequested) // 【精简点】合并异常路由
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperCloseTimeout, $"等待输入信号 {E_InPutName.晶圆夹爪右气缸闭合} 超时");
                }
                _logger.Warn($"[{MechanismName}] 夹爪闭合操作被外部取消");
                throw;
            }
            catch (Exception ex)
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.GripperCloseCylinderFailed, ex.Message);
            }
        }
        ...
        public async Task<bool> CheckStackedPieces(CancellationToken token)
        {
            ...
            catch (OperationCanceledException) // 【新增】外抛防吞噬
            {
                throw;
            }
            catch (Exception ex)
        ...
        public async Task<MechResult> MoveInitialNoScan(CancellationToken token = default)
        {
            try
            {
                ...
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialNoScanFailed, "Y轴移动到待机位失败");
            }
            catch (OperationCanceledException) // 【新增】外抛防吞噬
            {
                throw;
            ...
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialNoScanFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> MoveInitial(CancellationToken token = default)
        {
            try
            {
                ...
                {
                    bool? res1 = _io.ReadInput((int)E_InPutName.晶圆夹爪右铁环有无检测);
                    if (!res1.HasValue) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialResidualMaterial, $"获取输入信号 {E_InPutName.晶圆夹爪右铁环有无检测} 失败");

                    if (res1.Value)
                    {
                        return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialResidualMaterial, "机构已复位但检测到残留物料，请人工确认是否带料");
                    }
                    return MechResult.Success();
                }
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialFailed, "移动到待机位失败");
            }
            catch (OperationCanceledException) // 【新增】外抛防吞噬
            {
                throw;
            ...
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.MoveInitialFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> PutOverMove(CancellationToken token = default)
        {
            try
            {
                ...
                {
                    bool? res1 = _io.ReadInput((int)E_InPutName.晶圆夹爪右铁环有无检测);
                    if (!res1.HasValue) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PutOverMaterialStuck, $"获取输入信号 {E_InPutName.晶圆夹爪右铁环有无检测} 失败");

                    if (!res1.Value)
                    {
                        return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PutOverMaterialStuck, "卸料后依然检测到物料粘连");
                    }
                    return MechResult.Success();
                }
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PutOverMoveFailed, "移动到取出安全位置失败");
            }
            catch (OperationCanceledException) // 【新增】外抛防吞噬
            {
                throw;
            ...
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PutOverMoveFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> InitialMoveFeeding(CancellationToken token = default)
        {
            try
            {
                ...
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.InitialMoveFeedingFailed, "移动到晶圆取料位置失败");
            }
            catch (OperationCanceledException) // 【新增】外抛防吞噬
            {
                throw;
            ...
            {
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.InitialMoveFeedingFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> MoveDetection(CancellationToken token = default)
        {
            ...
            try
            {
                Task taskA = Task.Run(async () =>
                {
                    while (!linkedToken.IsCancellationRequested)
                    {
                        ...
                        if (_io.ReadInput((int)E_InPutName.晶圆夹爪右卡料检测) == true) return;
                    }
                }, linkedToken);
                ...
                Task taskB = Task.Run(async () =>
                {
                    ...
                    {
                        await Task.Delay(5, linkedToken);
                        if (_io.ReadInput((int)E_InPutName.晶圆夹爪右铁环有无检测) == true) return;
                    }
                }, linkedToken);
                ...
                {
                    cts.Cancel();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutTriggerFailed, "轴底层运动触发指令下发失败");
                }
                ...
                if (finishedTask.IsFaulted)
                {
                    ...
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutTriggerFailed, "状态检测任务发生异常");
                }

                if (finishedTask == taskA)
                {
                    await _yAxis.StopAsync();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutJamAlarm, "拉出过程中触发【卡料报警】，已紧急停止");
                }
                if (finishedTask == taskB)
                {
                    await _yAxis.StopAsync();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutDropAlarm, "拉出过程中触发【丢料报警】，已紧急停止");
                }
            ...
            catch (OperationCanceledException)
            {
                ...
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutTimeout, "Y轴拉出运动超时");
                }
            ...
            catch (Exception ex)
            {
                await _yAxis.StopAsync();
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PullOutTriggerFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult> FeedingMaterialToBox(CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    linked.Cancel();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackTriggerFailed, "送入运动触发失败");
                }
               ...
               token.ThrowIfCancellationRequested();
                ...
                if (finishedTask.IsFaulted)
                {
                    await _yAxis.StopAsync();
                    _logger.Warn(finishedTask.Exception?.InnerException?.Message ?? "状态检测任务发生未知异常");
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackTriggerFailed, "状态检测任务发生异常");
                }

                if (finishedTask == taskA)
                {
                    await _yAxis.StopAsync();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackJamAlarm, "送回过程中触发【卡料报警】，已紧急刹停");
                }
                if (finishedTask == taskB)
                {
                    await _yAxis.StopAsync();
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackDropAlarm, "送回过程中触发【丢料报警】，已紧急刹停");
                }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (timeoutcts.IsCancellationRequested)
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackTimeout, "送入运动超时");
                }
            ...
            catch (Exception ex)
            {
                await _yAxis.StopAsync();
                _logger.Warn(ex.Message);
                return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.PushBackTriggerFailed, ex.Message);
            }
        }
        ...
        public async Task<MechResult<List<string>>> CodeScanTigger(CancellationToken token = default)
        {
            try
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        var flag = await _dataModule.CheckCodeAsync(E_WorkSpace.工位2, str.Split('&').ToList(), token);
                        if (flag.IsSuccess)
                        {
                ...
                return MechResult<List<string>>.Fail(AlarmCodesExtensions.WS2Pulling.CodeScanFailed, "扫码失败或校验不合法，3次重试均未通过");
            }
