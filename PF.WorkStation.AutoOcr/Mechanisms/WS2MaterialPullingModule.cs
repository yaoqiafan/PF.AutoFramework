...
{
    /// <summary>
    /// 【工位2】推拉晶圆模组 (Wafer Pulling Mechanism)
    ///
    /// <para>物理架构：</para>
    ...
    public class WS2MaterialPullingModule(
        IHardwareManagerService hardwareManagerService,
        IParamService paramService,
        IContainerProvider provider,
        ILogService logger) : BaseMechanism(E_Mechanisms.工位2推拉晶圆模组.ToString(), hardwareManagerService, paramService, logger)
    {
        #region Enums (轴点枚举定义)
        ...
        public async Task<bool> CheckStackedPieces(CancellationToken token)
        {
            try
            {
                ...
                if (!res1.HasValue) return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.StackedPiecesDetected, $"获取输入信号 {E_InPutName.夹爪左叠料检测} 失败");

                if (res1.Value)
                {
                    return MechResult.Fail(AlarmCodesExtensions.WS2Pulling.StackedPiecesDetected, "检查到叠料，请人工确认是否叠料");
                }
                return MechResult.Success();
            }
        ...
        public async Task<MechResult> CheckProductDirection(E_WafeSize wafesize, CancellationToken token = default)
        {
            ...
            // 读取IO信号
            bool? res1 = wafesize == E_WafeSize._8寸 ? _io.ReadInput((int)E_InPutName.上晶圆右8寸铁环防反检测) : _io.ReadInput((int)E_InPutName.上晶圆右12寸铁环防反检测);
            if (res1.HasValue)
            {
                ...
                {
                    return wafesize == E_WafeSize._8寸 ? MechResult<bool>.Fail(AlarmCodesExtensions.WS2Pulling.Wafer8InchReversed, "8寸晶圆放反") : MechResult<bool>.Fail(AlarmCodesExtensions.WS2Pulling.Wafer12InchReversed, "12寸晶圆放反");
                }
            }
            else
            {
