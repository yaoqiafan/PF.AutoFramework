...
- **通讯调试面板**（v1.0.2 起）：按 `CommunicationCategory`/`CommunicationRole` 两级分类树展示所有 `ICommunication` 实例；提供 TCP 服务端、TCP 客户端、文件传输三个调试视图，含实时状态、测试收发、事件日志，均带"参数设置"入口（弹窗修改→存库→重新加载）
