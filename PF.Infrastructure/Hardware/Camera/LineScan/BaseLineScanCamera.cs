...
namespace PF.Infrastructure.Hardware.Camera.LineScan
{
    ...
    public abstract class BaseLineScanCamera : AttachedDeviceBase<IFrameGrabberCard>, ILineScanCamera
    {
        ...
        /// <summary>
        /// 从固定路径 <see cref="FeatureFilePath"/> 导入接线/成像属性。文件不存在时只记 Warn 并
        /// 沿用设备当前配置，不视为失败——现场首次上电、还没来得及导出文件时不应阻塞整机初始化。
        ///
        ...
        /// 即可，且必须在探测设备当前接线状态如 FrameTriggerMode 之前调用，否则探测到的是导入前的旧值）；
        /// 也可由 <see cref="ReimportFeatureFileAsync"/> 在调试面板按需手动重新触发。</para>
        /// </summary>
